...
class QNetwork(nn.Module):
    def __init__(self, observations: int, actions: int):
        super().__init__()
        self.network = nn.Sequential(nn.Linear(observations, 256), nn.ReLU(), nn.Linear(256, 256),
                                     nn.ReLU(), nn.Linear(256, actions))

    def forward(self, value: torch.Tensor) -> torch.Tensor:
        return self.network(value)


def train(episodes: int = 500, output: str = "runs/ew_dqn", seed: int = 7) -> Path:
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    env = EWSearchEnv(seed=seed)
    ...
    for episode in range(episodes):
        ...
        for _ in range(env.max_steps):
            ...
            with torch.no_grad():
                action = random.randrange(env.action_size) if random.random() < epsilon else int(
                    policy(torch.tensor(state, device=device).unsqueeze(0)).argmax())
            ...
            if len(replay) >= 256:
                ...
                optimizer.zero_grad(); loss.backward(); nn.utils.clip_grad_norm_(policy.parameters(), 10)
                optimizer.step(); writer.add_scalar("train/loss", loss.item(), global_step)
    ...
    torch.save({"model": policy.state_dict(), "observations": env.observation_size,
                "actions": env.action_size, "device": str(device)}, model_path)
    writer.close()
    return model_path


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--episodes", type=int, default=500)
    parser.add_argument("--output", default="runs/ew_dqn"); parser.add_argument("--seed", type=int, default=7)
    args = parser.parse_args(); print(train(args.episodes, args.output, args.seed))
